...
exports.getTimeSeriesMonth = function (req, res, next) {
...
exports.getTimeSeriesDay = function (req, res, next) {
  /*Getting time serires info
   *
   */
  let ticker = req.query.ticker
  ...
  axios
    .get(
      `https://cloud.iexapis.com/stable/stock/${
    ticker}/time-series/?token=pk_f606ae9814ec4d9e991aa1def338e260&range=today`
    )
    .then(response => {
      let timeSeriesData = response.data;
      let body = {}
      ...
      let dataPoints = []
      for (var i = 0; i < timeSeriesData.length; i++) {
        if (timeSeriesData[i].close !== null) {
          labels.push(
            new Date(`${timeSeriesData[i].date}T${timeSeriesData[i].minute}`)
          )
          dataPoints.push(
            timeSeriesData[i].close
          );
